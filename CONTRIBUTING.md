...
For Python packages, select the npm workspace adapter package in Changesets. For example, a `rownd-python` patch release uses:
...
Fix Rownd Python package behavior
```

The release PR generated by Changesets will update `packages/rownd-python/package.json` and `packages/rownd-python/CHANGELOG.md`.
The version script then syncs the same version into the package's `pyproject.toml`, runtime
`PLUGIN_VERSION`, and `uv.lock`. Running versioning locally therefore requires `uv`.
...
The version script synchronizes the Python metadata and lockfile. The release script builds the package and publishes
unpublished versions to PyPI.
PyPI publishing requires either trusted publishing for the GitHub workflow or a `PYPI_TOKEN` secret.
Use `npm run check:python-versions` to verify that each Python package's workspace version matches its
`pyproject.toml`, runtime `PLUGIN_VERSION`, and `uv.lock` package entry.
