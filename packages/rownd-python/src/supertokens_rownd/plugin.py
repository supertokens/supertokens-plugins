...
class RowndEmailDeliveryOverride:
    def __init__(self, original: Any, config: RowndPluginConfig, link_attr: str, target_path: str):
...
class RowndSMSDeliveryOverride:
    def __init__(self, original: Any, config: RowndPluginConfig):
        self.original = original
        self.config = config
...
def init(config: Optional[RowndPluginConfig] = None, **kwargs: Any) -> SuperTokensPlugin:
    if config is None:
    ...
    async def app_config_handler(request: Any, response: Any, *_: Any, **__: Any):
        return await handle_app_config(config, request, response)

    async def guest_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_guest_login(config, telemetry_client, request, response, user_context)

    async def migrate_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
    ...
    async def signout_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_signout(session, response)

    async def get_user_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_get_user(session, response)

    async def update_user_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_update_user(config, request, response, session, user_context)

    async def delete_user_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_delete_user(session, response)

    async def get_user_meta_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_get_user_meta(session, response)

    async def update_user_meta_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_update_user_meta(request, response, session)

    async def get_user_field_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
        return await handle_get_user_field(request, response, session)

    async def update_user_field_handler(request: Any, response: Any, session: Any, user_context: Dict[str, Any]):
    ...
    passwordless_override.apis = _passwordless_api_override(config)

    thirdparty_override = RecipePluginOverride()
    thirdparty_override.apis = _thirdparty_api_override(config)
    ...
    session_override.functions = _session_function_override(config)
    ...
    emailverification_override.apis = _emailverification_api_override()
...
def _passwordless_config_override(config: RowndPluginConfig):
    def override(original_config: Any) -> Any:
        ...
        def sms_override(original: Any) -> Any:
...
def _emailverification_api_override():
    def override(original: Any) -> Any:
        ...
        async def email_verify_post(
            ...
            result = await original_email_verify_post(token, session_, tenant_id, api_options, user_context)
            if getattr(result, "status", None) == "OK":
                await complete_pending_email_verification(result.user.recipe_user_id, result.user.email, user_context)
            return result
