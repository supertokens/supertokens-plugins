...
from .admin_methods import METHOD_KEY, execute_methods, plan_methods, validate_method_recovery
...
class Reconciliation:
    def __init__(self, client: Any, core_config: Any, context: JsonDict,
        ...
        self.store = AdministrativeCore(context)
    ...
    async def source_guard(self) -> None:
        for source_id, expected in self.profiles.items():
            if not same_json(await self.fetch(source_id), expected):
    ...
    async def prepare(self, source_id: Optional[str], selected_id: Optional[str], email: Optional[str]) -> tuple[JsonDict, Optional[JsonDict], Optional[str]]:
        ...
        if source_id:
            ...
            if recovery is not None:
                if (not isinstance(recovery, dict) or type(recovery.get("version")) is not int
                        or recovery["version"] != 1 or recovery.get("sourceId") != source_id
        ...
        if self.orphan:
            if winner != self.orphan["sourceId"] or self.orphan.get("winner", winner) != winner:
                raise AdministrativePolicyError("Orphan recovery election changed before handoff")
            pinned_profiles = self.orphan.get("profiles")
            if pinned_profiles is not None and not same_json(pinned_profiles, self.profiles):
        ...
        if checkpoint and "sourceProfile" in checkpoint and not same_json(checkpoint["sourceProfile"], self.profiles[winner]):
            raise AdministrativePolicyError("Checkpoint source changed")
        if checkpoint and "sourceProfile" in checkpoint and not same_json(checkpoint["operations"], plan_owner_operations(checkpoint, self.profiles[winner])):
        ...
        checkpoint = {"version": 2, "id": str(uuid.uuid4()), "sourceId": winner,
                      "sourceProfile": copy.deepcopy(self.profiles[winner]),
        ...
        checkpoint["operations"] = plan_owner_operations(checkpoint, self.profiles[winner])
    ...
    async def run(self, source_id: Optional[str], selected_id: Optional[str], email: Optional[str], dry_run: bool) -> JsonDict:
        ...
        missing = [m for m in source["loginMethods"] if not any(login_method_matches_import(existing, m) for existing in all_methods)]
        method_users = []
        if target:
        ...
        method_actions = await plan_methods(self.store, target, source, self.profiles[winner], method_users) if target else []
        occupied = await self.store.raw(target) if target else {}
        if plan:
            for recipe in plan["recipes"]:
        ...
        names = {"detach": "unlink_method", "promote": "create_primary", "link": "link_method",
                 ...
                 "create": "create_method", "retire": "review_provider_retirement", "update_email": "set_canonical_email"}
        ...
        if plan and plan["status"] in {"APPLYING", "RECONCILING"}:
            actions.append({"action": "update_migration_metadata"})
        if target and not occupied.get("rownd_email_recipe_user_id") and any(m.recipe_id == "passwordless" and m.email for m in current_methods):
        ...
        if target is None:
            ...
            for method in source["loginMethods"]:
                if method.get("email"):
                    await self.store.operation({"kind": "revoke_verification_tokens", "id": winner, "email": method["email"]}, target)
                    if not method["isVerified"] and await verification.is_email_verified(RecipeUserId(winner), method["email"], self.context):
                        raise AdministrativePolicyError("Fresh alias verification changed")
            await self.store.operation({"kind": "create_mapping", "id": target, "alias": winner}, target)
        ...
        if final_user is None or any(not any(login_method_matches_import(m, expected) for m in final_user.login_methods) for expected in source["loginMethods"]):
        ...
        for method in final_user.login_methods:
            if (verified_contact and method.email and method.email.lower() == verified_contact.lower()
                ...
                await self.source_guard()
                await self.store.operation({"kind": "verify_email", "id": method.recipe_user_id.get_as_string(), "email": method.email}, target)
        await self.progress("verification")
        await self.source_guard()
        current = await self.store.raw(target)
        occupied = {**occupied, **current}
        for method in final_user.login_methods:
            for literal in {method.recipe_user_id.get_as_string(), await self.store.immutable(method.recipe_user_id.get_as_string())}:
                occupied = {**await self.store.raw(literal), **occupied}
        patch = metadata_backfill(self.profiles[winner], occupied)
        patch["rownd_migration_complete"] = True
        email_methods = [m for m in final_user.login_methods if m.recipe_id == "passwordless" and m.email and m.email.lower() == self.profiles[winner]["data"].get("email", "").lower()]
        if len(email_methods) == 1:
            ...
            patch["rownd_email_recipe_user_ids"] = {**current.get("rownd_email_recipe_user_ids", {}),
                                                   "public": email_methods[0].recipe_user_id.get_as_string()}
        self.mutation_started = True
        await metadata.update_user_metadata(target, patch, self.context)
        mapping = await self.store.mapping(winner)
        if mapping is None or mapping["id"] != target:
            raise AdministrativePolicyError("Canonical mapping changed")
        if plan:
        ...
        if self.orphan:
            ...
            await metadata.update_user_metadata(winner, {ORPHAN_KEY: self.orphan}, self.context)
        self.result.update(status="OK", changed=bool(actions), actions=[a["action"] for a in actions],
...
async def reconcile_user(*, rownd_user_id: Optional[str] = None,
    ...
    try:
        ...
        if tenant_id != "public":
            raise AdministrativePolicyError("Administrative reconciliation requires public-only recipes")
