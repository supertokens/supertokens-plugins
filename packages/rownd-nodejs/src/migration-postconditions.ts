...
export async function assertMigrationOwnerGraph(
  user: User,
  tenantId: string,
  ...
  if (
    ...
    user.tenantIds.some(
      (tenant) =>
        !user.loginMethods.some((method) => method.tenantIds.includes(tenant)),
    ) ||
    user.loginMethods.some((method) =>
      method.tenantIds.some((tenant) => !user.tenantIds.includes(tenant)),
    )
  ) {
  ...
  for (const method of user.loginMethods) {
    const member = await SuperTokens.getUser(
    ...
    if (
      !member ||
      member.id !== user.id ||
      member.isPrimaryUser !== user.isPrimaryUser ||
      !member.loginMethods.some(
        (fresh) =>
          fresh.recipeUserId.getAsString() ===
            method.recipeUserId.getAsString() &&
          fresh.recipeId === method.recipeId &&
          JSON.stringify([...fresh.tenantIds].sort()) ===
            JSON.stringify([...method.tenantIds].sort()),
      )
    ) {
      throw new RowndMigrationPolicyError(
...
export async function reconcileAdministrativeEmailVerification(input: {
  ...
  if (!user)
    throw new RowndMigrationPolicyError(
      "MAPPING_TARGET_MISSING: verification owner is missing",
    );
  ...
  if (
    (canonicalId &&
      !user.loginMethods.some(
        (method) =>
          method.recipeUserId.getAsString() === canonicalId &&
          method.hasSameEmailAs(email),
      )) ||
    metadata.rownd_pending_verification?.some(
      (entry) =>
        entry.field === "email" && (entry.tenantId ?? "public") === tenantId,
    )
  ) {
    throw new RowndMigrationPolicyError(
  ...
  for (const method of user.loginMethods.filter(
    (method) =>
    ...
    const recipeUserId =
      mapping.status === "OK"
        ? SuperTokens.convertToRecipeUserId(mapping.externalUserId)
        : method.recipeUserId;
    const assertRecipeOwner = async () => {
      clearSuperTokensCoreCallCache(userContext);
      const currentMapping = await SuperTokens.getUserIdMapping({
        userId: recipeUserId.getAsString(),
        userIdType: "EXTERNAL",
        userContext,
      });
      const fresh = await SuperTokens.getUser(
        recipeUserId.getAsString(),
        userContext,
      );
      if (
        (mapping.status === "OK"
          ? currentMapping.status !== "OK" ||
            currentMapping.superTokensUserId !== mapping.superTokensUserId
          : currentMapping.status === "OK") ||
        !fresh ||
        fresh.id !== user.id ||
        !fresh.loginMethods.some(
          (entry) =>
            entry.recipeUserId.getAsString() === recipeUserId.getAsString() &&
            entry.tenantIds.includes(tenantId) &&
            entry.hasSameEmailAs(email),
        )
      ) {
        throw new RowndMigrationPolicyError(
    ...
    if (
      await EmailVerification.isEmailVerified(recipeUserId, email, userContext)
    )
      continue;
    await assertAuthenticatedMigrationSource(source, tenantId);
    ...
    if (token.status === "OK") {
      ...
      invalidateReconciliationReads("user", recipeUserId.getAsString());
      await assertAuthenticatedMigrationSource(source, tenantId);
      await assertMigrationMapping(
        internalUserId,
        source.externalUserId!,
      ...
      await assertRecipeOwner();
      const verified = await EmailVerification.verifyEmailUsingToken(
        tenantId,
        token.token,
        false,
        userContext,
      ...
      if (verified.status !== "OK")
        throw new RowndMigrationPolicyError(
          "SOURCE_VERIFICATION_DRIFT: email verification failed",
        );
    }
  }
  clearSuperTokensCoreCallCache(userContext);
...
export async function assertMigrationPostconditions(input: {
  ...
  userContext: JsonRecord;
  matchesMethod: (
    existing: User["loginMethods"][number],
    expected: ImportMethod,
  ) => boolean;
}) {
  const {
    internalUserId,
  ...
  const observe = (user: User | undefined): Failure | undefined => {
    ...
    if (user.id !== internalUserId && user.id !== source.externalUserId)
      return "unexpected_owner";
    for (const expected of importMethods) {
      const matches = user.loginMethods.filter((method) =>
        matchesMethod(method, expected),
      );
      if (matches.length === 0) return "missing_method";
      const scoped = matches.filter((method) =>
        method.tenantIds.includes(tenantId),
      );
      ...
      if (
        ...
        expected.email?.toLowerCase() === authenticatedEmail &&
        !scoped.some((method) => method.verified)
      ) {
    ...
    if (
      ...
      user.loginMethods.some(
        (method) =>
          method.tenantIds.includes(tenantId) &&
          method.hasSameEmailAs(authenticatedEmail) &&
          !method.verified,
      )
    ) {
      return "unverified_authenticated_email";
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const user = await SuperTokens.getUser(internalUserId, userContext);
    if (user && isAdministrativeMigration(source, tenantId))
      await assertMigrationOwnerGraph(user, tenantId, userContext);
    ...
    if (
      ...
      for (const method of user.loginMethods.filter(
        ...
        if (
          ...
          failure = "unverified_authenticated_email";
      }
    }
    ...
    const error = new RowndMigrationPolicyError(
      `Migrated login method postcondition failed: ${failure}`,
    );
    telemetry?.emit(
      "transition",
      `migration_postcondition_${failure}`,
      "error",
      error,
    );
