import {
  ...
  reconciliationMultitenancy as MultiTenancy,
...
function revocationLedgerId(internalUserId: string, tenantId: string) {
  return `rownd-provider-revocations-${createHash("sha256")
    .update(JSON.stringify([internalUserId, tenantId]))
    .digest("hex")}`;
}
...
async function checkpointProviderRevocation(
  ...
  await UserMetadata.updateUserMetadata(
    ledgerId,
    { [id]: { ...retired, internalUserId, tenantId } },
    userContext,
  );
  return async () => {
    await UserMetadata.updateUserMetadata(
...
export async function recoverProviderRevocations(
  internalUserId: string,
  rowndUserId: string,
  tenantId: string,
  userContext: JsonRecord,
  ...
  for (const [id, retired] of ledger) {
    ...
    const method = owner?.loginMethods.find((entry) =>
      entry.hasSameThirdPartyInfoAs({
        id: retired.provider,
        userId: retired.subject,
      }),
    );
    ...
    const revoked = await Session.revokeAllSessionsForUser(
      internalUserId,
      true,
      tenantId,
      userContext,
    );
    if (revoked.length > 0) onProgress?.();
    await UserMetadata.updateUserMetadata(
      ledgerId,
      { [id]: null },
      userContext,
...
async function inspectProviderRevocations(
  internalUserId: string,
  rowndUserId: string,
  tenantId: string,
  userContext: JsonRecord,
  ...
  return Object.entries(ledger).map(([id, value]) => {
    if (
      !isRecord(value) ||
      value.internalUserId !== internalUserId ||
      value.tenantId !== tenantId ||
      value.rowndUserId !== rowndUserId
    ) {
      throw new RowndMigrationPolicyError(
        "Provider revocation checkpoint target changed",
      );
    }
...
export function readRetirements(value: unknown): Retirement[] {
  ...
  if (
    ...
    value.some(
      (entry) =>
        !isRecord(entry) ||
        [entry.rowndUserId, entry.recipeUserId, entry.subject].some(
          (field) => typeof field !== "string" || !field,
        ) ||
        !["google", "apple"].includes(entry.provider as string) ||
        (entry.pendingTenantIds !== undefined &&
          (!Array.isArray(entry.pendingTenantIds) ||
            entry.pendingTenantIds.some((id) => typeof id !== "string"))),
    )
  ) {
    throw new RowndMigrationPolicyError(
      "Invalid Rownd provider retirement checkpoint",
    );
  }
...
export async function prepareRowndProviderRetirement(input: {
  ...
  const { source, user, metadata, internalUserId, tenantId, userContext } =
    input;
  const stored = (
    await UserMetadata.getUserMetadata(internalUserId, userContext)
  ).metadata;
  ...
  const unfinished = (
    await inspectProviderRevocations(
      internalUserId,
  ...
  for (const retired of unfinished) {
    if (
      !retirements.some((entry) => entry.recipeUserId === retired.recipeUserId)
    )
  ...
  return async () => {
    await recoverProviderRevocations(
      internalUserId,
      source.externalUserId!,
      tenantId,
      userContext,
    ...
    for (const retired of retirements) {
      ...
      if (retired.pendingTenantIds?.includes(tenantId)) {
        ...
        retired.pendingTenantIds = retired.pendingTenantIds.filter(
          (id) => id !== tenantId,
        );
      }
      const expected = source.loginMethods.find(
        (method) =>
          ...
          method.thirdPartyId === retired.provider,
      );
      if (
        !expected ||
        expected.recipeId !== "thirdparty" ||
        expected.thirdPartyUserId === retired.subject
      )
        continue;
      if (retired.rowndUserId !== source.externalUserId)
        throw new RowndMigrationPolicyError(
          "Rownd provider retirement source changed",
        );
      ...
      if (
        !fresh ||
        fresh.data.user_id !== source.externalUserId ||
        !isRowndMigrationProfileActive(fresh) ||
        resolveRowndProviderSubject(fresh, retired.provider) !==
          expected.thirdPartyUserId
      ) {
        throw new RowndMigrationPolicyError(
          "Rownd provider changed before retirement",
        );
      }
      clearSuperTokensCoreCallCache(userContext);
      await assertMigrationMapping(
      ...
      const checkpoint = readRetirements(
        (await UserMetadata.getUserMetadata(internalUserId, userContext))
          .metadata[key],
      );
      if (
        !checkpoint.some(
          (entry) =>
            entry.recipeUserId === retired.recipeUserId &&
            entry.subject === retired.subject &&
            entry.provider === retired.provider &&
            entry.rowndUserId === retired.rowndUserId,
        )
      ) {
        throw new RowndMigrationPolicyError(
          "Rownd provider retirement checkpoint changed",
        );
      }
      const current = await SuperTokens.getUser(internalUserId, userContext);
      const matchesReplacement = (
        method: SuperTokensUser["loginMethods"][number],
      ) =>
        method.recipeId === "thirdparty" &&
        method.tenantIds.includes(tenantId) &&
        method.hasSameThirdPartyInfoAs({
          id: retired.provider,
          userId: expected.thirdPartyUserId,
        });
      if (!current?.loginMethods.some(matchesReplacement))
        throw new RowndMigrationPolicyError(
          "Rownd replacement provider is not linked to the pinned account",
        );
      const owner = await SuperTokens.getUser(
        retired.recipeUserId,
        userContext,
      );
      const obsolete = owner?.loginMethods.find(
        (method) =>
          method.recipeUserId.getAsString() === retired.recipeUserId ||
          (retired.recipeUserId === internalUserId &&
            method.recipeUserId.getAsString() === source.externalUserId),
      );
      if (!obsolete) {
      ...
      if (
        owner!.id !== current.id ||
        obsolete.recipeId !== "thirdparty" ||
        !obsolete.hasSameThirdPartyInfoAs({
          id: retired.provider,
          userId: retired.subject,
        })
      ) {
        throw new RowndMigrationPolicyError(
          "Rownd obsolete provider ownership changed",
        );
      }
      if (
        !obsolete.tenantIds.includes(tenantId) &&
        obsolete.tenantIds.length > 0 &&
        !retired.pendingTenantIds?.includes(tenantId)
      )
        continue;
      retired.pendingTenantIds = [
        ...new Set([...(retired.pendingTenantIds ?? []), tenantId]),
      ];
      const acknowledgeRevocation = await checkpointProviderRevocation(
        internalUserId,
        tenantId,
        retired,
        userContext,
      );
      const beforeRemoval = readRetirements(
        (await UserMetadata.getUserMetadata(internalUserId, userContext))
          .metadata[key],
      );
      await UserMetadata.updateUserMetadata(
      ...
      if (obsolete.tenantIds.includes(tenantId)) {
        const result = await MultiTenancy.disassociateUserFromTenant(
          tenantId,
          SuperTokens.convertToRecipeUserId(retired.recipeUserId),
          userContext,
        );
        if (result.status !== "OK")
          throw new Error(
            `Failed to retire Rownd provider tenant: ${result.status}`,
          );
      }
      clearSuperTokensCoreCallCache(userContext);
      const after = await SuperTokens.getUser(internalUserId, userContext);
      const remaining = after?.loginMethods.find((method) =>
        method.hasSameThirdPartyInfoAs({
          id: retired.provider,
          userId: retired.subject,
        }),
      ...
      if (
        !after?.loginMethods.some(matchesReplacement) ||
        remaining?.tenantIds.includes(tenantId)
      ) {
        throw new RowndMigrationPolicyError(
          "Rownd provider tenant retirement failed",
        );
      }
      ...
      const afterRevocation = readRetirements(
        (await UserMetadata.getUserMetadata(internalUserId, userContext))
          .metadata[key],
      );
      await UserMetadata.updateUserMetadata(
      ...
      if (
        !final?.loginMethods.some(matchesReplacement) ||
        final.loginMethods.some(
          (method) =>
            method.tenantIds.includes(tenantId) &&
            method.hasSameThirdPartyInfoAs({
              id: retired.provider,
        ...
        throw new RowndMigrationPolicyError(
          "Rownd provider retirement postcondition failed",
        );
      }
    }
    clearSuperTokensCoreCallCache(userContext);
    ...
    for (const retired of retirements) {
      ...
      if (
        !owner?.loginMethods.some((method) =>
          method.hasSameThirdPartyInfoAs({
            id: retired.provider,
            userId: retired.subject,
          }),
    ...
    await UserMetadata.updateUserMetadata(
      internalUserId,
      {
        [key]: readRetirements(latest[key]).filter(
          (entry) =>
            !completed.has(entry.recipeUserId) ||
            (entry.pendingTenantIds?.length ?? 0) > 0,
        ),
      },
      userContext,
    );
...
export type ProviderIntroduction = Retirement & {
  tenantId: string;
  created: boolean;
  internalUserId: string;
};
...
function introductions(value: unknown): ProviderIntroduction[] {
  ...
  if (
    entries.some(
      (entry) =>
        typeof Reflect.get(entry, "tenantId") !== "string" ||
        typeof Reflect.get(entry, "created") !== "boolean",
    )
  )
    throw new RowndMigrationPolicyError(
      "Invalid provider introduction checkpoint",
    );
  return entries as ProviderIntroduction[];
}

export async function assertCurrentRowndProviders(
  source: SuperTokensUserImport,
  tenantId: string,
) {
  await assertAuthenticatedMigrationSource(source, tenantId);
  const providers = source.loginMethods.filter(
    (method) =>
      method.recipeId === "thirdparty" &&
  ...
  if (
    ...
    providers.some(
      (method) =>
        method.recipeId === "thirdparty" &&
        resolveRowndProviderSubject(fresh, method.thirdPartyId) !==
          method.thirdPartyUserId,
    )
  ) {
    throw new RowndMigrationPolicyError(
      "Rownd provider changed before linking or migration completion",
    );
  }
}
...
export async function checkpointProviderIntroduction(input: {
  ...
  created: boolean;
  userContext: JsonRecord;
  introduced: ProviderIntroduction[];
}) {
  if (!["google", "apple"].includes(input.provider)) return;
  const { internalUserId, userContext } = input;
  const introduction: ProviderIntroduction = {
    rowndUserId: input.source.externalUserId!,
  ...
  await UserMetadata.updateUserMetadata(
    input.recipeUserId,
    { [recipeIntroductionKey]: introduction },
    userContext,
  );
  const metadata = (
    await UserMetadata.getUserMetadata(internalUserId, userContext)
  ...
  if (
    !pending.some(
      (entry) =>
        entry.recipeUserId === input.recipeUserId &&
...
async function inspectProviderIntroductions(
  ...
  for (const method of target?.loginMethods ?? []) {
    ...
    if (
      entry.internalUserId === internalUserId &&
      !pending.some(
        (candidate) => candidate.recipeUserId === entry.recipeUserId,
      )
    )
      pending.push(entry);
  }
...
export async function finishProviderIntroductions(
  internalUserId: string,
  rowndUserId: string,
  userContext: JsonRecord,
  rollback: boolean,
  ...
  const pending =
    introduced ??
    (await inspectProviderIntroductions(internalUserId, userContext));
  if (pending.length === 0) return;
  await assertMigrationMapping(internalUserId, rowndUserId, userContext);
  for (const entry of pending) {
    if (entry.rowndUserId !== rowndUserId)
      throw new RowndMigrationPolicyError(
        "Provider introduction target changed",
      );
    ...
    const method = owner?.loginMethods.find(
      (candidate) =>
        candidate.recipeUserId.getAsString() === entry.recipeUserId,
    );
    const preservePinnedDonor =
      !entry.created &&
      owner?.id === target?.id &&
    ...
    if (method) {
      if (
        ...
        (owner!.isPrimaryUser && owner!.id !== target?.id)
      )
        throw new RowndMigrationPolicyError(
          "Provider introduction ownership changed",
        );
      ...
      if (entry.created || preservePinnedDonor) {
        if (method.tenantIds.includes(entry.tenantId)) {
          const removed = await MultiTenancy.disassociateUserFromTenant(
            entry.tenantId,
            method.recipeUserId,
            userContext,
          );
          if (removed.status !== "OK")
            throw new Error("Failed to quarantine introduced provider");
        }
    ...
    const remaining = after?.loginMethods.find(
      (candidate) =>
        candidate.recipeUserId.getAsString() === entry.recipeUserId,
    );
    if (
      (entry.created || preservePinnedDonor) &&
      remaining?.tenantIds.includes(entry.tenantId)
    )
      throw new RowndMigrationPolicyError(
        "Provider quarantine postcondition failed",
      );
    if (entry.created && remaining?.tenantIds.length === 0) {
      if (entry.recipeUserId === internalUserId)
        throw new RowndMigrationPolicyError(
          "Cannot quarantine the primary anchor",
        );
  ...
  await UserMetadata.updateUserMetadata(
    internalUserId,
    {
      [introductionKey]: introductions(latest[introductionKey]).filter(
        (entry) =>
          !pending.some(
            (completed) =>
              completed.recipeUserId === entry.recipeUserId &&
              completed.tenantId === entry.tenantId,
          ),
      ),
    },
    userContext,
...
export async function assertProviderSessionMembership(
  userId: string,
  recipeUserId: string,
  tenantId: string,
  userContext: JsonRecord,
  ...
  const primary = (await UserMetadata.getUserMetadata(internalId, userContext))
    .metadata;
  const alias =
    user.id !== internalId
      ? (await UserMetadata.getUserMetadata(user.id, userContext)).metadata
      : primary;
  if (
    !primary.original_rownd_user &&
    !alias.original_rownd_user &&
    primary.rownd_migration_complete !== true &&
    alias.rownd_migration_complete !== true &&
    !primary[introductionKey] &&
    !primary[key]
  )
    return;
  const method = user.loginMethods.find(
    (entry) =>
      entry.recipeUserId.getAsString() === recipeUserId ||
      (recipeUserId === internalId &&
        entry.recipeUserId.getAsString() === user.id),
  );
  if (
    !method ||
    (method.recipeId === "thirdparty" &&
      ["google", "apple"].includes(method.thirdParty!.id) &&
      !method.tenantIds.includes(tenantId))
  )
    throw new RowndMigrationPolicyError(
      "Migrated provider is no longer a member of this tenant",
    );
  const recipeMetadata = (
    await UserMetadata.getUserMetadata(recipeUserId, userContext)
  ).metadata;
  const pending =
  ...
  if (
    pending.some(
      (entry) =>
        entry.recipeUserId === recipeUserId &&
        (entry.internalUserId === undefined ||
          entry.internalUserId === internalId) &&
        (!entry.created || entry.tenantId === tenantId),
    )
  ) {
    throw new RowndMigrationPolicyError(
      "Migrated provider introduction is not yet committed",
    );
  }
}
